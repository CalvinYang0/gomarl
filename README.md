...
W&B logging is enabled by default in `src/config/default.yaml`. To sync online, run:

```shell
WANDB_MODE=online python3 src/main.py --config=group --env-config=sc2 with env_args.map_name=3s5z_vs_3s6z use_wandb=True wandb_mode=online
```
