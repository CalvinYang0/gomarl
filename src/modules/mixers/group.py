...
class Mixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        self.hyper_w1 = nn.ModuleList(
            nn.Sequential(
                nn.Linear(self.a_h_dim, self.grouping_hypernet_dim),
        ...
        self.hyper_w4 = nn.Sequential(
            nn.Linear(self.state_dim, self.hypernet_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.hypernet_dim, self.embed_dim),
        )
        ...
        for group_index, group_i in enumerate(self.group):
            ...
            for agent_i in group_i:
                ...
                sim_loss = th.sum(group_state_agent_i_sim * group_state_sim, dim=-1) / (
                    (th.sum(group_state_agent_i_sim ** 2, dim=-1) ** 0.5)
                    * (th.sum(group_state_sim ** 2, dim=-1) ** 0.5)
                )
                div_loss = th.sum(group_state_agent_i_div * group_state_div, dim=-1) / (
                    (th.sum(group_state_agent_i_div ** 2, dim=-1) ** 0.5)
                    * (th.sum(group_state_div ** 2, dim=-1) ** 0.5)
                )
