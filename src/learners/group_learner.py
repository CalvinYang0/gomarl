...
class GROUPLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer == "group":
            ...
            raise ValueError("Unsupported mixer={} for GROUPLearner.".format(args.mixer))
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        lasso_alpha = []
        for _ in range(len(w1_avg_list)):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("lasso_loss", lasso_loss.item() if hasattr(lasso_loss, "item") else lasso_loss, t_env)
    ...
    def change_group(self, batch: EpisodeBatch, change_group_i: int):
        ...
        if change_group_i == self.args.change_group_batch_num - 1:
            ...
            whether_group_changed = group_now != group_nxt
    ...
    def save_models(self, path):
        ...
        if self.mixer is not None:
            np.save("{}/group.npy".format(path), np.array(self.mixer.group, dtype=object), allow_pickle=True)
    ...
    def load_models(self, path):
        ...
        if self.mixer is not None:
            self.mixer.group = np.load("{}/group.npy".format(path), allow_pickle=True).tolist()
