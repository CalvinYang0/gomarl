...
# Restart KL80 under a unique name with per-slot gate-probability logging.
# This intentionally leaves all existing jobs untouched.
...
JOB_NAME="grf_counter_bayesg_kl80_slotdiag_s${SEED}"
RUN_NAME="grf_counter_bayesg_kl80_slotdiag_10m_s${SEED}"
MODEL_TYPE="grf_abs_dual_branch_binary_concrete_bayesg_kl80_hypercond"
...
printf 'submitted job=%s name=%s resources=%sc/%s time=%s run=%s\n' \
  "${job_id%%;*}" "$JOB_NAME" "$CPUS_PER_TASK" "$MEMORY" "$TIME" "$RUN_NAME"
