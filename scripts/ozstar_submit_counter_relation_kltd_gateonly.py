#!/usr/bin/env python3
"""Submit the Counter relation+KL80 gate-only auxiliary TD control.

Mutable output uses the personal home runtime. An active same-name job from
...
def main():
    ...
    if len(plans) != 1 or plans[0]["label"] != LABEL:
        raise RuntimeError("KL-TD gate-only job selection changed")
    ...
    manifest = paths["logs"] / (
        "counter_relation_kltd_gateonly_{}_{}.json".format(
