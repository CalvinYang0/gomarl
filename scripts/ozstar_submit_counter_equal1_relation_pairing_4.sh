...
# Four controlled equal1 ablations on GRF Counter:
#   equal1_temporal: relation=1 plus temporal parameter stability=1.
#   equal1_hard_train: relation=1 with deterministic p>0.5 STE gates.
#   equal1_episode_random: relation pairs are random within each episode.
#   equal1_global_random: relation pairs use all valid batch states, allowing
...
submit_one() {
  local label="$1" model_type="$2" temporal_coef="$3" pairing="$4"
  local job_name="grf_counter_mask_param_relation_${label}_s${SEED}"
  local run_name="grf_counter_mask_param_relation_${label}_10m_s${SEED}"
  local existing job_id args
  ...
  args="$common_args clean_mask_parameter_relation_temporal_coef=$temporal_coef clean_mask_parameter_relation_pairing=$pairing"
  ...
  printf 'submitted job=%s name=%s model=%s relation=1 temporal=%s pairing=%s\n' \
    "${job_id%%;*}" "$job_name" "$model_type" "$temporal_coef" "$pairing"
}

relation_model="grf_abs_dual_branch_binary_concrete_mask_parameter_relation_hypercond"
...
sleep "$SUBMIT_GAP_SECONDS"
submit_one \
  "equal1_episode_random" \
  "$relation_model" \
  "0.0" \
  "episode_random"
sleep "$SUBMIT_GAP_SECONDS"
submit_one \
  "equal1_global_random" \
  "$relation_model" \
  "0.0" \
  "global_random"
