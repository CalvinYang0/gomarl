#!/usr/bin/env python3
"""Preflight and submit the Counter Advantage objective 3x2 matrix."""
import json
import os
...
def main():
    ...
    if tuple(plan["label"] for plan in plans) != LABELS:
        raise RuntimeError("Advantage objective matrix selection changed")
    ...
    os.chdir(repo)
    subprocess.run(
        [sys.executable, "scripts/smoke_test_advantage_objective_six.py"],
        check=True,
    )
    user = run(["id", "-un"])
    ...
    manifest = paths["logs"] / (
        "counter_advantage_objective_six_{}_{}.json".format(
