...
SEEDS="${SEEDS:-1}"
MODELS="${MODELS:-rpg_global_public_private_bias_past_delta_token_transformer_hypercond rpg_global_public_private_bias_past_delta_token_transformer_topk_hypercond rpg_global_public_private_bias_past_delta_token_transformer_threshold_hypercond}"
GROUP_NAME="${GROUP_NAME:-3s5z_global_private_bias_delta_transformer_48h_s1}"
...
EXTRA_ARGS="${EXTRA_ARGS:-clean_target_topk=3 clean_target_threshold=0.5}"

cd "$REPO_DIR"
mkdir -p ozstar_logs

echo "== Submit 3s5z global mixed Transformer 48h runs =="
echo "repo: $REPO_DIR"
echo "map: $MAP_NAME"
echo "models: $MODELS"
echo "seeds: $SEEDS"
...
for model_type in $MODELS; do
  for seed in $SEEDS; do
    run_name="${MAP_NAME}_${model_type}_48h_ozstar_s${seed}"
