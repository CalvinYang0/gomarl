...
# temporal/perturbed-head controls.  The script ensures that all six jobs
# exist; an already-running job is reused rather than duplicated.
#
...
MEMORY="${MEMORY:-16G}"
CONTROL_MEMORY="${CONTROL_MEMORY:-24G}"
...
PERTURBED_HEAD_RELATIVE_STD="${PERTURBED_HEAD_RELATIVE_STD:-0.05}"
...
declare -a specs=(
  "perturbed_head_td_quality|grf_abs_dual_branch_binary_concrete_perturbed_head_td_quality_hypercond|$CONTROL_MEMORY|clean_perturbed_head_relative_std=$PERTURBED_HEAD_RELATIVE_STD clean_importance_alternating_training=False"
  "temporal_param_stability|grf_abs_dual_branch_binary_concrete_temporal_param_stability_hypercond|$CONTROL_MEMORY|clean_temporal_param_switch_margin=$SWITCH_MARGIN clean_importance_alternating_training=False"
  "temporal_param_small_change|grf_abs_dual_branch_binary_concrete_temporal_param_small_change_hypercond|$CONTROL_MEMORY|clean_temporal_param_switch_margin=$SWITCH_MARGIN clean_importance_alternating_training=False"
  "bayesg_kl80_keep|grf_abs_dual_branch_binary_concrete_bayesg_kl80_hypercond|$MEMORY|"
  "bayesg_kl80_threshold70|grf_abs_dual_branch_binary_concrete_bayesg_kl80_threshold70_hypercond|$MEMORY|"
  "bayesg_kl70|grf_abs_dual_branch_binary_concrete_bayesg_kl70_hypercond|$MEMORY|"
)
...
for spec in "${specs[@]}"; do
  ...
  IFS='|' read -r label model_type memory variant_args <<< "$spec"
  submit_one "$label" "$model_type" "$memory" "$variant_args"
