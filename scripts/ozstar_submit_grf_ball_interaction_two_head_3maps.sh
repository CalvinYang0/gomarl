...
# a shared 19-action Q head and zero-initialized self/ball-interaction residuals.
...
COUNTER_MEMORY="${COUNTER_MEMORY:-24G}"
OTHER_MEMORY="${OTHER_MEMORY:-16G}"
...
MODEL_TYPE="grf_abs_dual_branch_binary_concrete_td_only_ball_interaction_two_head_hypercond"
...
"$PYTHON_BIN" scripts/smoke_test_grf_ball_interaction_two_head.py
...
submit_one() {
  local scene="$1" env_config="$2" memory="$3"
  local job_name="grf_${scene}_ball2head_s${SEED}"
  local run_name="grf_${scene}_ball_interaction_two_head_10m_s${SEED}"
...
declare -a specs=(
  "counter|academy_counterattack_easy|$COUNTER_MEMORY"
  "pass|academy_pass_and_shoot_with_keeper|$OTHER_MEMORY"
  "3v1|academy_3_vs_1_with_keeper|$OTHER_MEMORY"
)
...
for spec in "${specs[@]}"; do
  ...
  IFS='|' read -r scene env_config memory <<< "$spec"
  submit_one "$scene" "$env_config" "$memory"
