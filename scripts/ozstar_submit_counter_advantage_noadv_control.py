#!/usr/bin/env python3
"""Preflight and submit the Counter no-Advantage control."""
import json
import os
...
LABELS = ("relation_noadv_augtd_nomasktd",)
...
def main():
    ...
    if tuple(plan["label"] for plan in plans) != LABELS:
        raise RuntimeError("No-Advantage control selection changed")
    ...
    os.chdir(repo)
    subprocess.run(
        [sys.executable, "scripts/smoke_test_advantage_noadv_control.py"],
        check=True,
    )
    user = run(["id", "-un"])
    ...
    manifest = paths["logs"] / (
        "counter_advantage_noadv_control_{}_{}.json".format(
