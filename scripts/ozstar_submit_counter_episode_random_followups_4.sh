...
# Four Counter follow-ups around equal1 episode-random relation:
...
random_model="grf_abs_dual_branch_binary_concrete_random_drop_aux_hypercond"

submit_one \
  "grf_counter_equal1_episode_random_sampled_gate_s${SEED}" \
  "grf_counter_equal1_episode_random_sampled_gate_10m_s${SEED}" \
  "$relation_model" \
  "clean_mask_parameter_relation_mask_source=sampled_gate"
sleep "$SUBMIT_GAP_SECONDS"

submit_one \
  "grf_counter_equal1_episode_random_randommask_k50_w10_timestep_s${SEED}" \
...
submit_one \
  "grf_counter_equal1_episode_random_randommask_k90_w10_timestep_s${SEED}" \
  "grf_counter_equal1_episode_random_randommask_k90_w10_timestep_10m_s${SEED}" \
  "$random_model" \
  "clean_mask_parameter_relation_mask_source=probability clean_random_drop_auxiliary_keep_probability=0.90 clean_random_drop_auxiliary_coef=1.0 clean_random_drop_auxiliary_scope=timestep clean_random_drop_auxiliary_combine_mode=multiply"
sleep "$SUBMIT_GAP_SECONDS"
