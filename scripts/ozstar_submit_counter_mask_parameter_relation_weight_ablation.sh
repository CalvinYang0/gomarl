...
# Two fixed-weight Counter runs for the mask-parameter relation auxiliary:
#   fixed5x:  relation/temporal/perturbed/KL = 50/250/5/5
#   equal1:   relation/temporal/perturbed/KL = 1/1/1/1
# The model type is otherwise identical across both runs.
...
submit_one() {
  local label="$1" relation_coef="$2" temporal_coef="$3" perturbed_coef="$4" gate_coef="$5"
  ...
  args="$common_args clean_mask_parameter_relation_coef=$relation_coef clean_mask_parameter_relation_temporal_coef=$temporal_coef clean_mask_parameter_relation_perturbed_head_coef=$perturbed_coef clean_mask_parameter_relation_gate_regularization_coef=$gate_coef"
  job_id=$(sbatch --parsable \
    ...
    --export=ALL,PYTHON_BIN="$PYTHON_BIN",CONFIG=clean_hyper,ENV_CONFIG=academy_counterattack_easy,MAP_NAME=academy_counterattack_easy,MODEL_TYPE=grf_abs_dual_branch_binary_concrete_mask_parameter_relation_hypercond,SEED="$SEED",T_MAX="$T_MAX",TEST_INTERVAL="$TEST_INTERVAL",BATCH_SIZE_RUN="$BATCH_SIZE_RUN",EXPECTED_BATCH_SIZE_RUN="$BATCH_SIZE_RUN",BATCH_SIZE="$BATCH_SIZE",BUFFER_SIZE="$BUFFER_SIZE",USE_WANDB="$USE_WANDB",WANDB_MODE="$WANDB_MODE",USE_CUDA="$USE_CUDA",RUN_NAME="$run_name",GROUP_NAME="$run_name",OMP_NUM_THREADS="$TORCH_NUM_THREADS",MKL_NUM_THREADS="$TORCH_NUM_THREADS",OPENBLAS_NUM_THREADS="$TORCH_NUM_THREADS",NUMEXPR_NUM_THREADS="$TORCH_NUM_THREADS",EXTRA_ARGS="$args" \
    scripts/ozstar_train_offline.sbatch)
  printf 'submitted job=%s name=%s weights=relation:%s temporal:%s perturbed:%s kl:%s\n' \
    "${job_id%%;*}" "$job_name" "$relation_coef" "$temporal_coef" "$perturbed_coef" "$gate_coef"
}

submit_one "fixed5x" 50.0 250.0 5.0 5.0
sleep "$SUBMIT_GAP_SECONDS"
submit_one "equal1" 1.0 1.0 1.0 1.0
