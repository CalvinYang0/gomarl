...
# Matched Corridor pair:
#   1) existing single public-entity Transformer baseline;
#   2) the same model plus a TD-only random condition-observation auxiliary.
# The auxiliary uses drop=0.8 (keep=0.2), weight=0.5, timestep resampling,
# and no mask-parameter relation loss.
...
submit_one() {
  ...
  job_id=$(sbatch --parsable \
    ...
    --export=ALL,PYTHON_BIN="$PYTHON_BIN",CONFIG=clean_hyper,ENV_CONFIG=sc2,MAP_NAME=corridor,MODEL_TYPE="$model_type",SEED="$SEED",T_MAX="$T_MAX",TEST_INTERVAL="$TEST_INTERVAL",BATCH_SIZE_RUN="$BATCH_SIZE_RUN",EXPECTED_BATCH_SIZE_RUN="$BATCH_SIZE_RUN",BATCH_SIZE="$BATCH_SIZE",BUFFER_SIZE="$BUFFER_SIZE",USE_WANDB="$USE_WANDB",WANDB_MODE="$WANDB_MODE",USE_CUDA="$USE_CUDA",RUN_NAME="$run_name",GROUP_NAME="$run_name",OMP_NUM_THREADS="$TORCH_NUM_THREADS",MKL_NUM_THREADS="$TORCH_NUM_THREADS",OPENBLAS_NUM_THREADS="$TORCH_NUM_THREADS",NUMEXPR_NUM_THREADS="$TORCH_NUM_THREADS",EXTRA_ARGS="$base_args $variant_args" \
    scripts/ozstar_train_offline.sbatch)
  echo "submitted job=${job_id%%;*} name=$job_name model=$model_type resources=${CPUS_PER_TASK}c/${MEMORY}"
}

submit_one \
  "corridor_transformer_only_baseline_s${SEED}" \
  "corridor_transformer_only_baseline_10m_s${SEED}" \
  "rpg_public_transformer_hypercond" \
  "clean_random_drop_auxiliary_coef=0.0"
sleep "$SUBMIT_GAP_SECONDS"

submit_one \
  "corridor_transformer_only_random_drop_d80_w05_timestep_s${SEED}" \
  "corridor_transformer_only_random_drop_d80_w05_timestep_10m_s${SEED}" \
  "rpg_public_transformer_random_drop_aux_hypercond" \
  "clean_random_drop_auxiliary_keep_probability=0.20 clean_random_drop_auxiliary_coef=0.5 clean_random_drop_auxiliary_scope=timestep clean_random_drop_auxiliary_combine_mode=replace"
