...
# Five one-seed Counter experiments on the same non-three-head dual-branch
# architecture. Only the gate auxiliary objective changes.
...
PERTURBATION_RELATIVE_STD="${PERTURBATION_RELATIVE_STD:-0.05}"
PERTURBATION_PROBE_TIMESTEPS="${PERTURBATION_PROBE_TIMESTEPS:-2}"
...
if [[ "$CANCEL_PREVIOUS" == "1" ]]; then
  echo "== Cancel active jobs from this repo except entity-three-head jobs =="
  while IFS='|' read -r job_id job_name; do
    ...
    case "$job_name" in
      *entity3head*|*entity_three_head*)
        echo "keep three-head job=$job_id name=$job_name"
        ;;
      *)
        echo "cancel previous job=$job_id name=$job_name"
        scancel "$job_id"
        ;;
    esac
...
declare -a specs=(
  "bayesg_kl20_keep|grf_abs_dual_branch_binary_concrete_bayesg_kl20_hypercond|$MEMORY"
  "bayesg_kl80_keep|grf_abs_dual_branch_binary_concrete_bayesg_kl80_hypercond|$MEMORY"
  "hard_concrete_l0|grf_abs_dual_branch_hard_concrete_l0_hypercond|$MEMORY"
  "perturb_param_importance|grf_abs_dual_branch_binary_concrete_perturb_param_importance_hypercond|$PARAM_MEMORY"
  "gradient_importance|grf_abs_dual_branch_binary_concrete_gradient_importance_hypercond|$MEMORY"
)
