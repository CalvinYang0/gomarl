...
# Corridor counterparts of the three Counter gate-auxiliary controls:
#   1) perturbed generated-head TD quality;
#   2) adjacent generated-parameter stability;
#   3) small adjacent generated-parameter change suppression.
# The TD loss trains the full model; each auxiliary loss updates only the gate.
...
submit_one() {
  ...
  local job_name="corridor_${label}_s${SEED}"
  local run_name="corridor_${label}_10m_s${SEED}"
