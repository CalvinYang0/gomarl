...
# Two strict ablations of equal1 on GRF Counter:
#   equal1_temporal: relation=1 plus temporal parameter stability=1.
#   equal1_hard_train: relation=1, with deterministic p>0.5 hard gates in
#                      training and a straight-through sigmoid gradient.
...
submit_one() {
  local label="$1" model_type="$2" temporal_coef="$3"
  ...
  args="$common_args clean_mask_parameter_relation_temporal_coef=$temporal_coef"
  ...
  printf 'submitted job=%s name=%s model=%s relation=1 temporal=%s\n' \
    "${job_id%%;*}" "$job_name" "$model_type" "$temporal_coef"
}
