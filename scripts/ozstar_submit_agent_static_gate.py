#!/usr/bin/env python3
"""Preflight and append the independent per-agent Counter mask control."""
import json
import os
...
def main():
    ...
    os.chdir(repo)
    subprocess.run(
        [sys.executable, "scripts/smoke_test_agent_static_gate.py"], check=True
    )
    ...
    for row in run(["squeue", "-u", user, "-h", "-o", "%i|%j"]).splitlines():
        ...
        if retained is not None:
            raise RuntimeError("Duplicate active per-agent mask job")
        retained = job_id
    ...
    manifest = logdir / "agent_static_gate_{}_{}.json".format(
