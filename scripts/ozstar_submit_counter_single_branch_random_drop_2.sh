...
# Apply the successful TD-only random observation auxiliary to two single-
# branch Counter controls. Both use drop=0.8 (keep=0.2), weight=0.5, and a
# freshly sampled mask at every timestep. No mask-parameter relation is used.
...
submit_one \
  "grf_counter_mlp_full_random_drop_d80_w05_timestep_s${SEED}" \
  "grf_counter_mlp_full_random_drop_d80_w05_timestep_10m_s${SEED}" \
  "grf_abs_mlp_relation_random_drop_aux_hypercond"
sleep "$SUBMIT_GAP_SECONDS"

submit_one \
  "grf_counter_transformer_only_random_drop_d80_w05_timestep_s${SEED}" \
  "grf_counter_transformer_only_random_drop_d80_w05_timestep_10m_s${SEED}" \
